...
def _engine(**kwargs) -> CumulativeScoreEngine:
    redis = kwargs.pop("redis", Mock())
    prefix = kwargs.pop("prefix", "ac")
    return CumulativeScoreEngine(
        prefix,
        redis,
...
def test_cumulative_score_engine_stores_dependencies():
    normalizer = LowercaseNormalizer()
    tokenizer = WhitespaceTokenizer()
    redis = Mock()

    client = CumulativeScoreEngine("ac", redis, normalizer=normalizer, tokenizer=tokenizer)
...
def test_click_increments_score_on_all_prefix_keys():
    ...
    assert redis.zincrby.call_count == 8
    for key in expected_keys:
        redis.zincrby.assert_any_call(key, 1.0, "Red Apple")
...
def test_rescore_increments_all_prefix_keys():
    ...
    assert redis.zincrby.call_count == 8
    for key in expected_keys:
        redis.zincrby.assert_any_call(key, 2.5, "Red Apple")
